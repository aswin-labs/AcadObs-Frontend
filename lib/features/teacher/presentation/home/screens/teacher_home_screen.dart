...
class _TeacherHomeScreenState extends State<TeacherHomeScreen> {
  ...
  Widget _buildUpdatesTabSection(BuildContext context) {
  ...
  Widget _buildNoticesTab(BuildContext context) {
    return Consumer<NoticeProvider>(
      builder: (context, provider, _) {
        final notices = provider.latestNotices;
        if (provider.isLoading && notices.isEmpty) {
          return const CommonShimmerTile();
        }
        if (notices.isEmpty) {
          return emptyScreen(
        ...
        return ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            return EventCard(
              ...
              time: TimeFormatter.formatTime(event.createdAt ?? DateTime.now()),
            );
          },
        );
      },
      ...
      builder: (context, provider, _) {
        ...
        return ListView.separated(
          ...
          itemBuilder: (context, index) {
            ...
            final formattedDate = DateFormat('dd-MM-yy').format(newsItem.date);
            return NewsCard(
              news: newsItem,
              button: () {
                context.pushNamed(RouteConstants.newsScreen, extra: newsItem);
              },
              date: formattedDate,
...
Widget _buildTimeTableSection(BuildContext context) {
  return Consumer<TimeTableProvider>(
    builder: (context, provider, _) {
      ...
      // Adjusts card width depending on device
      double maxCrossAxisExtent;
      if (screenWidth >= 1200) {
        maxCrossAxisExtent = 250; // large screen (desktop)
