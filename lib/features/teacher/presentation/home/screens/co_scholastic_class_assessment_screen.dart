...
class CoScholasticClassAssessmentScreen extends StatefulWidget {
  final ClassGradeModel classGrade;

  const CoScholasticClassAssessmentScreen({
    super.key,
    required this.classGrade,
  });

  @override
  State<CoScholasticClassAssessmentScreen> createState() =>
      _CoScholasticClassAssessmentScreenState();
}

class _CoScholasticClassAssessmentScreenState
    extends State<CoScholasticClassAssessmentScreen> {
  int? _selectedExamId;
  ...
  Future<void> _initData() async {
    ...
    // Pre-fetch co-scholastic areas using first student if available
    if (studentProvider.students.isNotEmpty) {
      await myClassProvider.fetchCoScholasticAreas(
        studentId: studentProvider.students.first.id,
      );
    }
    ...
    if (mounted && termExamProvider.termExams.isNotEmpty) {
      setState(() {
        ...
        final name =
            termExamProvider.termExams.first['exam_name']?.toString() ?? '';
        final year =
            termExamProvider.termExams.first['education_year']?.toString() ??
                '';
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<int>(
                              ...
                              items: termExamProvider.termExams.map((exam) {
                                ...
                                final label =
                                    year.isNotEmpty ? '$name - $year' : name;
                              ...
                              onChanged: (newExamId) {
                                ...
                                final selected =
                                    termExamProvider.termExams.firstWhere(
                                  (e) => e['id'] == newExamId,
                                  orElse: () => {},
                                );
                                final name =
                                    selected['exam_name']?.toString() ?? '';
                                final year =
                                    selected['education_year']?.toString() ??
                                        '';
                                setState(() {
                                  _selectedExamId = newExamId;
                                  _selectedExamName = year.isNotEmpty
                                      ? '$name - $year'
                                      : name;
                                });
                              },
                    ...
                    child: filteredStudents.isEmpty
                        ...
                        : ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final originalIndex = allStudents
                                  ...
                                  onTap: () {
                                    if (_selectedExamId == null) {
                                      ScaffoldMessenger.of(context)
                                        ...
                                        const SnackBar(
                                          content: Text(
                                            "Please select a Term Exam first",
                                          ),
                                        ),
                                      );
                                      return;
                                    ...
                                    context.pushNamed(
                                      ...
                                      extra: StudentCoScholasticRatingArgs(
