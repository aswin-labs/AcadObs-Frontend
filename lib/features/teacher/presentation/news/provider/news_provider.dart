...
import 'package:acadobs/features/parents/data/services/news_service.dart';
...
class NewsProvider extends ChangeNotifier {
  ...
  Future<void> fetchHomeLatestNews({int limit = 3}) async {
    ...
    notifyListeners();
    try {
      final response = await _newsService.fetchNews(pageNo: 1, limit: limit);
      if (response.statusCode == 200) {
        final data = response.data['news'];
        final fetched = List<News>.from(data.map((e) => News.fromJson(e)));
        fetched.sort((a, b) => b.date.compareTo(a.date));
  ...
  void loadMore() {
    if (_hasMore && !_isLoading) {
      fetchNews(pageNo: _currentPage + 1);
    }
  }
}
