...
class StudentLeaveRequestProvider extends ChangeNotifier {
  ...
  //today leave request
  List<LeaveModel> get todayLeave => _leaveRequests.where((e){
    final createdAt = e.createdAt;
    if(createdAt == null) return false;
    final now = DateTime.now();
    return now.year == createdAt.year && now.month == createdAt.month && 
        now.day == createdAt.day;
  }).toList()
  ..sort((a,b) => b.createdAt!.compareTo(a.createdAt!));

  //yeasterday leave request
  List<LeaveModel> get yesterdayLeave => _leaveRequests.where((e){
    final createdAt = e.createdAt;
    if(createdAt == null) return false;
    final yesterday = DateTime.now().subtract(const Duration(days: 1));
    return yesterday.year == createdAt.year && yesterday.month==createdAt.month&&
  ...
  Future<void> fetchAllStudentLeaveRequests({
    ...
    try {
      ...
      final response = await StudentLeaveRequestServices()
          .fetchAllStudentLeaveRequests(pageNo: _currentPage);
  ...
  Future<void> createStudentLeaveRequest({
    ...
    try {
      ...
      if (response.statusCode == 201) {
        await fetchAllStudentLeaveRequests(forceRefresh: true);
