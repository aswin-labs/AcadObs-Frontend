...
class EventDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
     final String formattedDate = events.date != null
    ...
    //     : 'N/A';
    return Scaffold(
      appBar: CommonAppBar(title: capitalizeEachWord(events.title.toString()),isBackButton: true,),
      body: Padding(
  ...
  child: ClipRRect(
    ...
    child: CachedNetworkImage(
      imageUrl: BaseUrls.media +
                        MediaEndpoints.eventImages +
                        events.file.toString(),
      placeholder: (context, url) => const Center(child: CircularProgressIndicator()),
      errorWidget: (context, url, error) => const Center(
        child: Icon(Icons.event, size: 85, color: Colors.black),
      ),
      fit: BoxFit.cover,
    ),
  ),
          ...
          Text("There is an event based on the notification that you have received",
          style: TextStyle(
